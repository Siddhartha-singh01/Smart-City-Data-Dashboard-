...
def air():
    ...
    u = (
        ...
        f"latitude={lat}&longitude={lon}&hourly=pm10,pm2_5,us_aqi,ozone,carbon_monoxide&timezone=auto"
    )
    r = requests.get(u, timeout=20)
    if r.status_code != 200:
        return jsonify({"error":"air quality fetch failed"}), r.status_code
    j = r.json()
    h = j.get('hourly') or {}
    return jsonify({
        "time": h.get('time') or [],
        "pm2_5": h.get('pm2_5') or [],
        "pm10": h.get('pm10') or [],
